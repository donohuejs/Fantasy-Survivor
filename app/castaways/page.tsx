...
export default function CastawaysPage() {
  ...
  return <main className="inner-page">
    ...
    <section className="castaway-section">
      <div className="castaway-grid">{game.castaways.map((castaway) => {const tribe=tribeForCastaway(game,castaway.id);return <article className="castaway-card" key={castaway.id}>
        <img src={castaway.imageUrl} alt={`${castaway.name}, Survivor castaway`} loading="lazy"/>
        <div className="castaway-body"><div className="castaway-status"><span className={`status-dot ${castaway.status === 'active' ? 'active' : ''}`}/>{castaway.status === 'active' ? 'Still alive' : 'Voted out'}</div><div className="castaway-tribe"><span aria-hidden="true" style={tribe?{backgroundColor:tribe.color}:undefined}/>{tribe?.name??'Tribe not assigned'}</div><h3>{castaway.name}</h3><p className="castaway-meta">Age {castaway.age} · {castaway.occupation}</p><p className="castaway-bio">{castaway.bio}</p><div className="castaway-score"><span>{game.draftPicks.some((pick) => pick.castawayId === castaway.id) ? 'Drafted' : 'Undrafted'}</span><strong>{castawayScores[castaway.id] ?? 0} <small>pts</small></strong></div></div>
      </article>})}</div>
    </section>
  </main>;
}
