...
export function CastawayPool(){
  ...
  return <section className="castaway-pool-section" aria-labelledby="castaway-pool-heading">
    ...
    <div className="castaway-pool-grid">{board.items.map(item=>{const castaway=game.castaways.find(candidate=>candidate.id===item.castawayId);if(!castaway)return null;return <article className={`castaway-pool-card pool-${item.status}`} key={castaway.id}>
      ...
      <small>{castaway.occupation}</small>
