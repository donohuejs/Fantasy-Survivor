...
export default function Draft() {
  ...
  return <main className="inner-page">
    ...
    <CastawayPool/>
    <section className="draft-order-section"><div className="section-title"><div><p className="eyebrow dark">Assigned slots</p><h2>Reverse Season {game.season.number-1} finish order</h2></div></div><div className="draft-slot-grid">{[...game.players].sort((a,b)=>a.draftSlot-b.draftSlot).map((player)=><article key={player.id}><strong>{player.draftSlot}</strong><span>{(player.uid||player.email)?<PlayerName id={player.id} name={player.name} history={game.history}/>: 'Awaiting assignment'}<small>{(player.uid||player.email)?`Season ${game.season.number-1} finish: ${player.priorFinish}`:'Player signup pending'}</small></span></article>)}</div></section>
