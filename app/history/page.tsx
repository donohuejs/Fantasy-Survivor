...
export default function History(){
  ...
  return <main className="inner-page"><SiteHeader active="/history" subtitle="League history"/>
    ...
    <section className="history-content">
      ...
      <div className="history-scroll"><table className="history-table"><caption>{selected==='all'?'Career totals, ranked by points':`Final results for Season ${selected}`}</caption><thead><tr><th scope="col">Rank</th><th scope="col">Player</th><th scope="col">Seasons</th><th scope="col">Wins</th><th scope="col">Average finish</th><th scope="col">Total points</th></tr></thead><tbody>{board.map((player,index)=><tr key={player.profileId}><td>{selected==='all'?1+board.slice(0,index).filter(p=>p.total>player.total).length:player.averageFinish}</td><th scope="row"><PlayerName id={player.profileId} name={player.name} history={game.history}/></th><td>{player.seasons}</td><td>{player.wins}</td><td>{player.averageFinish.toFixed(1)}</td><td><strong>{player.total.toFixed(2)}</strong></td></tr>)}</tbody></table></div>
