...
export function SeasonManager(){
  ...
  return <section className="season-admin signup-admin"><p className="eyebrow dark">Year after year</p><h2>Season {game.season.number} · {locked?'Results locked':'In progress'}</h2><p>Lock final results only after the finale. The next draft uses reverse finish order for Round 1, then snakes back in Round 2, so the designated Survivor winner keeps the first Round 2 pick. Player profiles stay linked; paid checkmarks and bonus points reset each season.</p>
    {message&&<p role="status">{message}</p>}
    {!locked?<details><summary>Review and lock final standings</summary><p>Tied scores need an explicit final order from the game master—no automatic alphabetical tiebreaker.</p><ol className="final-result-list">{standings.map(row=>{const tied=standings.filter(r=>r.score===row.score);return <li key={row.profileId}><span><PlayerName id={row.profileId} name={row.name} history={game.history}/> · {row.score.toFixed(2)} pts</span>{tied.length>1&&<label>Final position for <PlayerName id={row.profileId} name={row.name} history={game.history}/><select value={tieRanks[row.profileId]??''} onChange={e=>setTieRanks({...tieRanks,[row.profileId]:e.target.value})}><option value="">Resolve tie…</option>{tied.map(r=><option key={r.finish} value={r.finish}>{r.finish}</option>)}</select></label>}</li>;})}</ol><button disabled={busy||game.draft.status!=='complete'||!game.scoreEvents.length} onClick={finalize}>Lock final results</button></details>:<><h3>Season {game.season.number+1} draft order</h3><ol>{next.map(player=><li key={player.id}><PlayerName id={player.id} name={player.name} history={game.history}/> · prior finish {player.priorFinish}</li>)}</ol><button disabled={busy} onClick={rollover}>Open Season {game.season.number+1}</button></>}
