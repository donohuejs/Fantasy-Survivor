...
export default function Rules() {
  const {game}=useGame();
  const groups = Object.groupBy(game.categories.filter(category=>!category.retired),(category) => category.group);
  return <main className="inner-page">
    ...
    <section className="rules-explanations"><article><h2>Before the merge</h2><p>Every active member of the winning tribe receives +2 for tribal immunity, including sit-outs. After a pre-merge Tribal Council, the game master can award +1 in one submission to every active member remaining on that tribe; a castaway voted out before the merge receives −1. The first time a tribe attends Tribal Council, every assigned member receives points equal to that episode number; for Episode 1, that is +1.</p></article><article><h2>Rewards and stacking</h2><p>The primary tribal reward is +2 for every active member of the winning tribe and the secondary tribal reward is +1. An individual reward winner receives +2, selected participants receive +1, and the winner keeps +2 even when giving up the reward spot. Sitting out remains −1 and can stack with immunity or reward points.</p></article><article><h2>Episode controls</h2><p>The game master awards “Still on the island” +1 once per episode to the complete active roster before changing an eliminated castaway’s status. One-time castaway bonuses are direct, noted score events. Majority-vote and blindside points are available only at or after the saved merge episode.</p></article></section>
    <section className="rules-layout"><aside><div className="rule-summary"><span>${game.season.entryFee}</span><strong>Entry fee</strong><p>League entry for the new season.</p></div><div className="rule-summary"><span>3</span><strong>Castaways drafted</strong><p>Three rounds, with round three eligible for the blind-pick bonus.</p></div><div className="rule-summary"><span>1.25×</span><strong>Blind-pick bonus</strong><p>Keep the blind pick and that castaway’s score receives a 25% boost.</p></div></aside><div className="rules-groups">{Object.entries(groups).map(([group,items]) => <article key={group}><h2>{group}</h2><div>{items?.map((category) => <div className="scoring-rule" key={category.id}><span className={category.points < 0 ? 'negative' : ''}>{category.dynamicPoints==='episode'?'+Episode':`${category.points > 0 ? '+' : ''}${category.points}`}</span><p><strong>{category.label}</strong></p></div>)}</div></article>)}</div></section>
  </main>;
}
