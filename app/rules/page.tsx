...
export default function Rules() {
  ...
  return <main className="inner-page">
    ...
    <section className="rules-layout"><aside><div className="rule-summary"><span>$20</span><strong>Entry fee</strong><p>League entry for the new season.</p></div><div className="rule-summary"><span>3</span><strong>Castaways drafted</strong><p>Three rounds, with round three eligible for the blind-pick bonus.</p></div><div className="rule-summary"><span>1.25×</span><strong>Blind-pick bonus</strong><p>Keep the blind pick and that castaway’s score receives a 25% boost.</p></div></aside><div className="rules-groups">{Object.entries(groups).map(([group,items]) => <article key={group}><h2>{group}</h2><div>{items?.map((category) => <div className="scoring-rule" key={category.id}><span className={category.points < 0 ? 'negative' : ''}>{category.points > 0 ? '+' : ''}{category.points}</span><p><strong>{category.label}</strong></p></div>)}</div></article>)}</div></section>
  </main>;
}
